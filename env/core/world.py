...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        self.current_available_food_positions = set()
        # rewards
        ...
        #     row[0] = row[-1] = self.WALL
        self.world[:, 0] = self.WALL  # left wall
        self.world[:, -1] = self.WALL  # right wall
        self.world[0] = self.WALL  # top wall
        self.world[-1] = self.WALL  # bottom wall
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = (random.randrange(1, self.size[0] - 1), random.randrange(1, self.size[1]) - 1)
    ...
    def init_food(self):
        ...
        for i in range(1, self.size[0] - 1):
            for j in range(1, self.size[1] - 1):
                if (i, j) not in self.snake.blocks:
                    self.current_available_food_positions.add((i, j))
        if not self.custom:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            elif new_snake_head in self.snake.blocks[3:]:
                self.snake.alive = False
